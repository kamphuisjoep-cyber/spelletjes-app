...
function addPipe() { let top=Math.random()*(flappyCanvas.height-pipeGap-80)+40; pipes.push({x:flappyCanvas.width, top:top, bottom:top+pipeGap}); }
...
function drawFlappy() {
    flappyCtx.clearRect(0,0,flappyCanvas.width,flappyCanvas.height);
    flappyCtx.fillStyle="#ffeb3b"; flappyCtx.fillRect(bird.x,bird.y,bird.w,bird.h);
    flappyCtx.fillStyle="#388e3c"; pipes.forEach(p=>{
        flappyCtx.fillRect(p.x,0,pipeWidth,p.top); flappyCtx.fillRect(p.x,p.bottom,pipeWidth,flappyCanvas.height-p.bottom);
    });
    flappyCtx.fillStyle="#0277bd"; flappyCtx.font="28px Arial"; flappyCtx.fillText("Score: "+score,20,40);
}
...
function endFlappy(){ gameOver=true; document.getElementById('score').textContent="Game Over! Je score: "+score+". Druk op spatie om opnieuw te spelen."; }

// ================= TETRIS =================
// … [Hier komt de Tetris code zoals eerder, maar met id=“tetrisCanvas”] …

// ================= SNAKE =================
// … [Hier komt de Snake code zoals eerder, maar met id=“snakeCanvas”] …

// ================= GALGJE =================
// … [Hier komt de Galgje code zoals eerder] …
